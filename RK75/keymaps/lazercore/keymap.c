...
static uint16_t encoder_button_keycode_for_active_layer(uint8_t encoder, uint8_t button) {
    ...
    for (int8_t layer = LAYER_STATE_BITS - 1; layer >= 0; --layer) {
        if (!(active & ((layer_state_t)1 << layer))) {
            continue;
        }

        uint16_t keycode = dynamic_keymap_get_encoder_button((uint8_t)layer, encoder, button);
...
KC_EQL,   KC_BSPC,  KC_DEL,
    ...
    [3] = LAYOUT(
        _______,  _______,  ENCODER_BTN, _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,
...
_______,  _______,  _______,
    ...
    [5] = LAYOUT(
        DFU_MODE_KEY, _______, ENCODER_BTN, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,
 _______, _______,
        _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,  _______,
_______,  _______,  _______,
    ...
    switch (keycode) {
        case ENCODER_BTN:
            ...
            if (record->event.pressed) {
                uint16_t mapped = KC_NO;
#    if defined(VIA_ENABLE)
                mapped = encoder_button_keycode_for_active_layer(0, 0);
#    elif defined(ENCODER_MAP_ENABLE)
                mapped = pgm_read_word(&encoder_button_map[0][0][0]);
#    endif
                if (mapped != KC_NO && mapped != KC_TRANSPARENT) {
                    tap_code16(mapped);
                }
            }
            return false;
